...
const AdminWhitelist = () => {
  ...
  const [newItem, setNewItem] = useState({ email: '', phoneNumber: '', note: '' });
  ...
  const handleToggle = async (item) => {
    try {
      await api.put(`/api/admin/whitelist/${item.whitelist_id}`, {
  ...
  return (
    <div>
                ...
                {whitelist.length === 0 ? (
                  ...
                  whitelist.map((item) => (
                    <tr key={item.whitelist_id}>
                      ...
                      <td className="text-sm text-base-content/60">{item.note || '-'}</td>
                      <td>
                        <Toggle
        ...
        <div className="modal modal-open">
          <div className="modal-box">
            ...
            <div className="space-y-4">
              ...
              <div className="form-control">
                <label className="label"><span className="label-text">備註 (選填)</span></label>
                <input
                  type="text"
                  className="input input-bordered"
          ...
          <div className="modal-backdrop" onClick={() => { setShowAdd(false); setError(''); }}></div>
        </div>
      )}
      ...
      <ConfirmDialog
        isOpen={!!deleteItem}
        title="確認刪除"
        message={`確定要從白名單移除「${deleteItem?.email || deleteItem?.phone_number}」嗎？`}
