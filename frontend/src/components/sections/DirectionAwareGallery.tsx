...
/** 根據滑鼠進入位置判斷方向 */
function detectDir(e: React.MouseEvent<HTMLDivElement>, el: HTMLDivElement): Dir {
...
const OVERLAY_VARIANTS: Record<Dir, Variants> = {
  top:    { initial: { y: "-100%", opacity: 0.8 }, animate: { y: "0%", opacity: 1 }, exit: { y: "-100%", opacity: 0 } },
  bottom: { initial: { y: "100%",  opacity: 0.8 }, animate: { y: "0%", opacity: 1 }, exit: { y: "100%",  opacity: 0 } },
  left:   { initial: { x: "-100%", opacity: 0.8 }, animate: { x: "0%", opacity: 1 }, exit: { x: "-100%", opacity: 0 } },
  right:  { initial: { x: "100%",  opacity: 0.8 }, animate: { x: "0%", opacity: 1 }, exit: { x: "100%",  opacity: 0 } },
};
  ...
  return (
    <section className="py-16 sm:py-20 px-4">
      ...
      <div className="text-center mb-10 sm:mb-12" data-aos="fade-up">
        <span className="text-gold text-xs uppercase tracking-widest">Moments</span>
        <h2 className="mt-2 text-2xl sm:text-3xl font-light text-white/90">精彩回顧</h2>
        <p className="mt-2 text-sm text-white/40">Hover 探索每張照片背後的故事</p>
      </div>
